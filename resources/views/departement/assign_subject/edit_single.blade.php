        ...
        <section class="content pb-5">
            <div class="container">
                <div class="card shadow-sm rounded-4 border-0 mx-auto" style="max-width: 600px;">
                    <form method="POST" action="{{ route('departement.assign_subject.update_single', $record->id) }}"
                        novalidate>
                        @csrf
                        @method('PUT')
                        <div class="card-body">
                                ...
                                <select id="academic_year_id" name="academic_year_id"
                                    ...
                                    @foreach ($academicYears as $year)
                                        <option value="{{ $year->id }}"
                                            {{ $selectedAcademicYear == $year->id ? 'selected' : '' }}>{{ $year->name }}
                                        </option>
                                    @endforeach
                                </select>
                                ...
                                <select id="subject_id" name="subject_id"
                                    ...
                                    @foreach ($subjects as $subject)
                                        <option value="{{ $subject->id }}"
                                            {{ $record->subject_id == $subject->id ? 'selected' : '' }}>
                            ...
                            <div class="mb-4">
                                ...
                                <select id="status" name="status"
                                    ...
                                    <option value="0" {{ $record->status == 0 ? 'selected' : '' }}>Actif</option>
                                    <option value="1" {{ $record->status == 1 ? 'selected' : '' }}>Inactif</option>
                                </select>
                                @error('status')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
